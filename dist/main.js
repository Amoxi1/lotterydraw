...
var startBtn = document.querySelector('.suduku__item--start') // 顺时针滚动
...
var setSpeed = function setSpeed (num) {
  ...
  if (config.cnt <= roundNum) {
    // 转速不变
    setTimeout(setSpeed, config.speed, num)
  }

  if (config.cnt > roundNum && config.cnt < roundNum + num) {
    // 最后一圈转速变慢
    config.speed = config.speed + (config.cnt - roundNum + 8) * 5
    setTimeout(setSpeed, config.speed, num)
  }

  if (config.cnt >= roundNum + num) {
    // i>总格数+最后一圈格数时停止
    var timer2 = null
